...
function nse_equity_openint(baseurl,downloadpath,unzippath,outputpath,fromd,tod,logpath,sn,count)
{
try
{
	Ext.getCmp('pbar3').updateText('NSE Equity Open interest Processing is started . . .');
	var url = require('url');
	...
	downloadAndProcessNSEEquityOpenint(strStart_date, strEnd_date, tempPath, outputpath);
    



function downloadAndProcessNSEEquityOpenint(strStart_date, strEnd_date, tempPath, outputpath)
{
	
	var urlStockinfo = "http://www.nseindia.com/content/nsccl/fao_participant_oi_"+strEnd_date+".csv";
    var tempFilePath = tempPath+"NSEtemp_openint_"+strEnd_date+".csv";
	var fname = "NSEtemp_openint_"+strEnd_date+".csv";
//	console.log('\n url = '+urlStockinfo);
	request
	(
		{
			...
			if(response != null)
			{
				if(response.statusCode != null || response.statusCode != "")
				{
					if (!error && response.statusCode == 200)
					{
						...
						downloadNSEEquityOpenint(200, urlStockinfo, tempFilePath, fname, outputpath);
					}
					else
					{
						downloadNSEEquityOpenint(200, urlStockinfo, tempFilePath, fname, outputpath);
...
function downloadNSEEquityOpenint(statuscode, urlStockinfo, tempFilePath, fname, outputpath_block)
 {
	if(statuscode==200)
	{
		Ext.getCmp('pbar3').updateText('Downloading '+fname);
		...
		req.on('end', function()
		{
			...
			}, function(err, result)
			{
				...
				{         
					...
					//console.log('\nop '+outputpath_block);
					/*if(fs.existsSync(outputpath_block+'NSE_fao_participant_oi_reports.csv'))
					{
						// file exist - dont append header
						for(var i=1 ; i<arrFileData.length-1;i++)
						{
							fs.appendFileSync(outputpath_block+'NSE_fao_participant_oi_reports.csv', arrFileData[i]+'\n','utf-8', function (err) {
								if (err) throw err;
							});
			...
			document.getElementById("om").innerHTML+='<br>'+fname+' is downloaded...';
                        loadprogressbar();
                        setTimeout(function(){process_bse_bulk(baseurl,downloadpath,unzippath,outputpath,fromd,tod,logpath,sn,count);},1000);
