...
function process_nse_ffix(count,n,baseurl,downloadpath,unzippath,outputpath,logpath,fromd,tod,sn)
{
try{
    ...
    try{
        ...
        async.parallel({
           ...
           data1 : function(callback){
                if (fs.existsSync(unzippath+'ffix'+d+m+y+'.csv')) {
					Ext.getCmp('pbar3').updateText('Processing ffix'+d+m+y+'.csv');
					Ext.getCmp('pbar3').updateProgress(0.18);
                    fs.readFile(unzippath+'ffix'+d+m+y+'.csv','utf-8', function(err, data){
			...
			if(count.length-2 == n)
			{
				step(
					function zero(){
						...
						fs.appendFileSync(outputpath+'NSE_marketcapreport.csv', arrFileData[0]+'\n','utf-8',  function(err){
							if(err){document.getElementById("om").innerHTML+="<br/>"+err;}
						});
						return this;
					},
					function one(){
							...
							fs.appendFileSync(outputpath+'NSE_marketcapreport.csv','\n','utf-8',  function(err)
							{
								if(err){document.getElementById("om").innerHTML+="<br/>"+err;}
							});
						return this;
					...
					function two(){
							...
							fs.appendFileSync(outputpath+'NSE_marketcapreport.csv','For date : '+d+'/'+m+'/'+y+'\n','utf-8',  function(err)
							{
								if(err){document.getElementById("om").innerHTML+="<br/>"+err;}
							});
						return this;
					...
					function four(){
						...
						{
								fs.appendFileSync(outputpath+'NSE_marketcapreport.csv', arrFileData[i]+'\n','utf-8',  function(err){
							if(err){document.getElementById("om").innerHTML+="<br/>"+err;}
							});
						}
						return this;
				...
				Ext.getCmp('pbar3').updateText('Processing ffix'+d+m+y+'.csv completed');
...
//write array to output file			
    ...
    document.getElementById("om").innerHTML+='<br/>ffix'+d+mon+yy+'.csv is processed..';
    addlog(logpath, 'ffix'+d+mon+yy+'.csv is processed..');
    n--;
	
        if(n>=0)
        {
			loadprogressbar();
            setTimeout(function (){process_nse_ffix(count,n,baseurl,downloadpath,unzippath,outputpath,logpath,fromd,tod,sn)}, 1000);
