...
function process_mcxindex(count,n,baseurl,downloadpath,unzippath,outputpath,logpath)
{
    ...
    try{
        async.parallel({
            data1 : function(callback){
                if (fs.existsSync(downloadpath+'mcxindia/MCXIndex'+n+'.csv')) {
                    fs.readFile(downloadpath+'mcxindia/MCXIndex'+n+'.csv','utf-8', function(err, data){
                        if(err){
...
*/
                ...
                var filename = outputpath1+'MCX_Indices_MCXIndex.csv';
                if(fs.existsSync(filename))
                {
                    for(i=1;i<arrFileData.length;i++)
                    {
                         fs.appendFileSync(filename,getname(n)+','+getname(n)+','+getcorrect(arrFileData[i][0])+','+arrFileData[i][1]+','+arrFileData[i][2]+','+arrFileData[i][3]+','+arrFileData[i][4]+',0,0'+'\n','UTF-8', function (err) {
                            if(err){
                                document.getElementById("om").innerHTML+="<br/>"+err;
                            }
