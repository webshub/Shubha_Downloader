...
function process_ninja(outputpath,m, baseurl)
{
	//console.log('\n op path = '+outputpath);
	
	var outputpath_amibriker=outputpath;
	outputpath_amibriker = outputpath_amibriker.replace(/std_csv/g,'ninja');
	...
	fs.readdir(outputpath,function(err, files)
	{
		...
		startNinja(files, 0, outputpath,outputpath_amibriker,m,baseurl);
			
	});
	}


function startNinja(files, n, outputpath, outputpath_amibriker,m, baseurl)
{
		...
		if(err){document.getElementById("om").innerHTML+="<br/>"+err;}
                        ...
                        //if (fs.existsSync(outputpath+files[n]) && (!fs.existsSync(outputpath_amibriker+files[n])|| files[n].search("yahoort")==0|| files[n].search("yahoo_eod")==0|| files[n].search("Yahoo_Fundamental")==0|| files[n].search("NSE_Indices_nseindex")==0|| files[n].search("NSE_Advance_D_nsead")==0|| files[n].search("MCX_Indices_MCXSpotIndex")==0|| files[n].search("MCX_Indices_MCXIndex")==0|| files[n].search("BSE_Indices_bseindex")==0))
			{
                                ...
                                for(var i=0 ; i<arrFileData.length-1;i++)
				{
					if(i > 0)
					{
						fs.appendFileSync(outputpath_amibriker+files[n], +arrFileData[i][2]+','+('00:'+arrFileData[i][3].split(":")[1]+':00').toString()+','+arrFileData[i][4]+','+arrFileData[i][5]+','+arrFileData[i][6]+','+arrFileData[i][7]+','+arrFileData[i][8]+'\n','utf-8', function (err) {
							if (err) throw err;
						});
					}
                                        /*else
				...
				{
					fs.appendFileSync(outputpath_amibriker+files[n], arrFileData[i][0]+','+arrFileData[i][2]+','+arrFileData[i][3]+','+arrFileData[i][4]+','+arrFileData[i][5]+','+arrFileData[i][6]+','+arrFileData[i][7]+'\n','utf-8', function (err) {
						if(err){document.getElementById("om").innerHTML+="<br/>"+err;}
					});
				}
                                ...
                                for(var i=1 ; i<arrFileData1.length-1;i++)
				{
					if(i > 1)
					{
						fs.appendFileSync(outputpath_amibriker+files[n], arrFileData1[i][0]+','+arrFileData1[i][2]+','+arrFileData1[i][3]+','+arrFileData1[i][4]+','+arrFileData1[i][5]+','+arrFileData1[i][6]+','+arrFileData1[i][7]+'\n','utf-8', function (err) {
							if (err) throw err;
						});
					}else
					{
						fs.writeFileSync(outputpath_amibriker+files[n], arrFileData1[i][0]+','+arrFileData1[i][2]+','+arrFileData1[i][3]+','+arrFileData1[i][4]+','+arrFileData1[i][5]+','+arrFileData1[i][6]+','+arrFileData1[i][7]+'\n','utf-8', function (err) {
							if (err) throw err;
						});
			...
			if(files.length > n)
			{
                                loadprogressbar();
				Ext.getCmp('pbar3').updateText('Processing '+files[n]+' completed');
				setTimeout(function(){startNinja(files, n, outputpath, outputpath_amibriker,m, baseurl)},1000);
			}else
			{	
				document.getElementById("om").innerHTML+="<br/>Ninja finished";
                                /************/
                                visit('ninjatrader.html');
                                /***************/
	...
	document.getElementById("om").innerHTML+="<br/>Ninja "+files[n]+" is processed..";
