...
function process_bse_blockdeal(baseurl,downloadpath,unzippath,outputpath,fromd,tod,logpath,sn,count)
{
try
{
	...
	downloadAndProcessBSEBlock(dateFormateForBulk, tempPath, outputpath);
    



function downloadAndProcessBSEBlock(dateFormateForBulk, tempPath, outputpath)
{
	//http://www.bseindia.com/stockinfo/BulkBlockFiles/Block_26Dec2012.csv 
	var urlStockinfo = "http://www.bseindia.com/stockinfo/BulkBlockFiles/Block_"+dateFormateForBulk+".csv";
			...
			if(response != null)
			{
				if(response.statusCode != null || response.statusCode != "")
				{
					if (!error && response.statusCode == 200)
					{
						...
						downloadBlockData(200, urlStockinfo, tempFilePath, fname, dateFormateForBulk, outputpath);
					}
					else
					{
						downloadBlockData(404, urlStockinfo, tempFilePath, fname, dateFormateForBulk, outputpath);
					}
				}else
				{
					downloadBlockData(404, urlStockinfo, tempFilePath, fname, dateFormateForBulk, outputpath);
...
function downloadBlockData(statuscode, urlStockinfo, tempFilePath, fname, dateFormateForBulk, outputpath_block)
 {
	if(statuscode==200)
	{
		Ext.getCmp('pbar3').updateText('Downloading '+fname);
		...
		req.on('end', function()
		{
			...
			async.parallel(
			{
               data1 : function(callback){
                    if (fs.existsSync(tempFilePath)) {
						...
						if(err){document.getElementById("om").innerHTML+="<br/> An error occured while processing bseblockdeals.csv";}
                        callback(err, data);
                    });
                    }else
                    {
			...
			}, function(err, result)
			{
				...
				{           
					...
					var arrFileData = new Array();
                                            ...
                                            //console.log('\nop '+outputpath_block);
                                            if(fs.existsSync(outputpath_block+'bseblockdeals.csv'))
                                            {
                                                    // file exist - dont append header
                                                    for(var i=1 ; i<arrFileData.length-1;i++)
                                                    {
                                                            fs.appendFileSync(outputpath_block+'bseblockdeals.csv', arrFileData[i]+'\n','utf-8', function (err) {
                                                                    if(err){document.getElementById("om").innerHTML+="<br/> An error occured while processing bseblockdeals.csv";}
                                                            });
			...
			document.getElementById("om").innerHTML+='<br>'+fname+' is downloaded...';
                        loadprogressbar();
                        setTimeout(function(){process_nse_bc(count,count.length-1,baseurl,downloadpath,unzippath,outputpath,logpath,fromd,tod,sn);},1000);
