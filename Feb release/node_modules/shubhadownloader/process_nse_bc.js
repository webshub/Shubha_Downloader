...
function process_nse_bc(count,n,baseurl,downloadpath,unzippath,outputpath,logpath,fromd,tod,sn)
{
try
{
    ...
    try{
        ...
        async.parallel({
           ...
           data1 : function(callback){
                if (fs.existsSync(unzippath+'Bc'+d+m+y+'.csv')) {
					Ext.getCmp('pbar3').updateText('Processing Bc'+d+m+y+'.csv');
					Ext.getCmp('pbar3').updateProgress(0.05);
                    fs.readFile(unzippath+'Bc'+d+m+y+'.csv','utf-8', function(err, data){
			...
			if(count.length-2 == n)
			{
				step(
					function zero(){
						...
						fs.appendFileSync(outputpath+'NSE_corporate_actions.csv', arrFileData[0]+'\n','utf-8',  function(err){
							if(err){document.getElementById("om").innerHTML+="<br/>"+err;}
						});
						return this;
					},
					function one(){
							...
							fs.appendFileSync(outputpath+'NSE_corporate_actions.csv','\n','utf-8',  function(err)
							{
								if(err){document.getElementById("om").innerHTML+="<br/>"+err;}
							});
						return this;
					...
					function two(){
							...
							fs.appendFileSync(outputpath+'NSE_corporate_actions.csv','For date : '+d+'/'+m+'/'+y+'\n','utf-8',  function(err)
							{
								if(err){document.getElementById("om").innerHTML+="<br/>"+err;}
							});
						return this;
					...
					function four(){
						...
						{
								fs.appendFileSync(outputpath+'NSE_corporate_actions.csv', arrFileData[i]+'\n','utf-8',  function(err){
							if(err){document.getElementById("om").innerHTML+="<br/>"+err;}
							});
						}
						return this;
				...
				Ext.getCmp('pbar3').updateText('Processing bc'+d+m+y+'.csv completed');
...
//write array to output file			
    ...
    document.getElementById("om").innerHTML+="<br/>Bc"+d+mon+yy+".csv is processed..";
    addlog(logpath, 'Bc'+d+mon+yy+'.csv is processed..');
    n--;
        if(n>=0)
        {
			loadprogressbar();
                        setTimeout(function (){process_nse_bc(count,n,baseurl,downloadpath,unzippath,outputpath,logpath,fromd,tod,sn)}, 2000);
...
process_nse_bh(count,count.length-1,baseurl,downloadpath,unzippath,outputpath,logpath,fromd,tod,sn);
}
